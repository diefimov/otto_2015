...
def load_train_data(path):
    ...
    return X, y, y_coded, ids, tfidf
    
def load_test_data(path, tfidf):
    df = pd.read_csv(path)
    X = df.values.copy()
...
X, y, y_coded, ids_train, tfidf = load_train_data(train_file)
X_test, ids_test = load_test_data(test_file, tfidf)
num_classes = len(y[0])
...
for e in range(epoch):
    ...
    if cv == 0:
        ...
        for i in range(num_classes):
            ...
            clf=ExtraTreesClassifier(n_jobs=-1, n_estimators=300, criterion="entropy", max_features=30, max_depth=25, random_state=seed)
            clf.fit(X[index_shuffle,:],yi)
            preds_epoch['Class_'+str(i+1)+'_et'] = clf.predict_proba(X_test)[:,1]
        ...
        for train_index, valid_index in skf:
            ...
            for i in range(num_classes):
                ...
                preds_fold['Class_'+str(i+1)+'_et'] = clf.predict_proba(X_valid)[:,1]
